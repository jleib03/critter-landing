...
## Original offer release verification (website commit `f469a89`)
...
## Approved TTP capability-showcase polish

The follow-up replaces the static right-hand hero preview with six illustrated capabilities: Snapshot, Journey, Lead capture, Programs, Task lists, and Togo. Each has its own explanation and graphic, clearly labeled as an illustrative product highlight. The controls support direct selection, Previous/Next wrapping, and explicit Pause/Play. Automatic rotation is seven seconds. Manual selection and keyboard entry stop rotation until Play; mouse hover, an offscreen panel, and a hidden document temporarily hold it. Reduced motion starts paused. Touch Play works without sticky compatibility hover.

The shared Togo icon now uses the same Lucide `Dog` outline as Hub's current Togo widget, retaining the existing named/default exports and size/color/className contract. Unlabeled inline icons are decorative; a supplied accessible label makes the standalone icon an image. The initial snapshot retains Critter's own mark. No bitmap, package dependency, or illustration outcome statistic was added.

Tests were updated first: the old glyph failed the current-Dog render assertion and the prior page failed the no-navigation assertion. Actual responsive checks then found and drove fixes for the narrow logo pair, graphic overflow, the small desktop headline row, and touch compatibility hover. Headlines, descriptions, and graphic panels are measured independently; a stable outer height is asserted across all six slides. Desktop navigation remains on generic pages and TTP signup/sign-in callbacks remain intact.

Final verification after the clean candidate rebuild:

- `npm test`: **109 checks passed**, actual server rendering including icon exports/accessibility, no `/ttp` navigation, retained generic navigation, six direct capability choices, and the existing offer/CTA regressions. Log: `/private/tmp/bl403-website-showcase-unit.log`.
- `npm run build`: **passed**, including production compile, lint/type validation, and static generation. Log: `/private/tmp/bl403-website-showcase-build.log`. Existing Browserslist and optional image-optimizer advisories do not block the build; dependencies are unchanged.
- `node tests/public-site.browser.cjs`: **258 checks passed** at **320, 390, 768, and 1440px**, including all six slides, independent headline/body/graphic fit, stable outer height, controls, seven-second rotation, hover and focus semantics, pointer/touch Pause/Play, offscreen/hidden holds, reduced motion, preserved generic navigation, actual signup-boundary clicks, and no browser runtime errors. Log: `/private/tmp/bl403-website-showcase-browser.log`.
- `./node_modules/.bin/tsc --noEmit`: **passed** with empty diagnostics log `/private/tmp/bl403-website-showcase-types.log`. `git diff --check`: passed.
- Final captures: `/private/tmp/bl403-website-browser/capability-{snapshot,journey,lead-capture,programs,task-lists,togo}-{320,390,1440}.png`; complete pages `/private/tmp/bl403-website-browser/{ttp,pricing,faqs}-{320,390,768,1440}.png`; first hero viewports `/private/tmp/bl403-website-browser/ttp-hero-{320,1440}.png`. Desktop hero and narrow Snapshot/Togo/Lead/Tasks captures were visually inspected. Earlier failed diagnostic captures are not release evidence.

The browser harness uses the real built Next page, real pointer/keyboard/touch events, a local virtual clock for intervals, and actual IntersectionObserver notifications before testing offscreen holds. Document visibility changes and the remote partner logo are synthetic test boundaries. An outside pointer position is verified in-document rather than using Chromium's exact `(0,0)` edge, which can omit `pointerleave`. No React state or timer instrumentation remains in the final test. These are local website tests, not deployed Hub signup, live provider, billing, or onboarding end-to-end proof. No remote publication was performed.
